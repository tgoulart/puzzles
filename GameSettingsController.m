    ...
    if (self) {
        ...
        while (config_items[num].type != C_END) {
            ...
            if (config_items[num].type == C_CHOICES) {
                NSString *sval = [NSString stringWithUTF8String:config_items[num].u.choices.choicenames];
    ...
    switch (indexPath.section) {
        case 0:
            if (type == CFG_SETTINGS) {
                ...
                NSAssert(config_items[indexPath.row].type == C_STRING, @"Expected config_item type to be C_STRING.");
...
{
    NSAssert(config_items[index].type == C_CHOICES, @"This can only be called from GameSettingsChoiceController, which only gets created for C_CHOICES");

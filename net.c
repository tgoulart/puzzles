...
#define ROT(x, n) (((n)&3) == 0 ? (x) : ((n)&3) == 1 ? A(x) : ((n)&3) == 2 ? F(x) : C(x))
        ...
        for (y = 0; y < h; y++)
            for (x = 0; x < w; x++)
            {
                if (x + 1 < w && ((ROT(index(params, tiles, x, y), 2) ^
                                   index(params, tiles, x + 1, y)) &
                                  L))
                    mismatches++;
                if (y + 1 < h && ((ROT(index(params, tiles, x, y), 2) ^
                                   index(params, tiles, x, y + 1)) &
                                  U))
                    mismatches++;
            }
                            ...
                            int x, int y, int button)
{
	 ...
	 */
        ...
        action = (button == LEFT_BUTTON ? ROTATE_LEFT : button == RIGHT_BUTTON ? ROTATE_RIGHT : TOGGLE_LOCK);
             ...
             action == ROTATE_180)
    {
         ...
         */
        sprintf(buf, "%c%d,%d", (int)(action == ROTATE_LEFT ? 'A' : action == ROTATE_RIGHT ? 'C' : 'F'), tx, ty);
        return dupstr(buf);
    }
             ...
             action == MOVE_ORIGIN_AND_SOURCE || action == MOVE_CURSOR)
    {
        ...
        if (action == MOVE_ORIGIN || action == MOVE_ORIGIN_AND_SOURCE)
        {
            if (state->wrapping)
            {
                ...
                return nullret; /* disallowed for non-wrapping grids */
        }
                      ...
                      unsigned long tile, float angle)
{
    ...
    for (d = 1, dsh = 0; d < 16; d *= 2, dsh++)
    {
        ...
        {
            int x, y, w, h;
            int col = (pass == 0 || edgetype == 1 ? COL_WIRE : edgetype == 2 ? COL_POWERED : COL_ERR);
    ...
    for (pass = 0; pass < 2; pass++)
    {
        ...
        if (endtype)
        {
            ...
            col = (pass == 0 || endtype == 3 ? COL_WIRE : endtype == 2 ? COL_POWERED : COL_ENDPOINT);
                        ...
                        float t, float ft)
{
    ...
    tx = ty = -1;
    last_rotate_dir = dir == -1 ? oldstate->last_rotate_dir : state->last_rotate_dir;
                              ...
                              const game_state *newstate, int dir, game_ui *ui)
{
     ...
     */
    last_rotate_dir = dir == -1 ? oldstate->last_rotate_dir : newstate->last_rotate_dir;
    if (last_rotate_dir)
        return ROTATE_TIME;
